# Integrated BI — Backend Django

API REST de la plateforme **Integrated BI** (Django 5 + DRF + PostgreSQL), conçue dans le cadre du PFE 2026 pour SOTIFibre.

🌐 **API en production** : [sotifibre-backend.onrender.com](https://sotifibre-backend.onrender.com)
📖 **Documentation OpenAPI** : [sotifibre-backend.onrender.com/api/docs/](https://sotifibre-backend.onrender.com/api/docs/) (Swagger UI)
...
Créer un fichier `.env` à la racine de `sotifibre_backend_django/` :
```env
SECRET_KEY=ton_secret_django_50+_caracteres
DEBUG=True
DATABASE_URL=postgres://user:pass@localhost:5432/sotifibre_db
...
```bash
curl -X POST https://sotifibre-backend.onrender.com/api/auth/jwt/token/ \
  -H "Content-Type: application/json" \
  -d '{"email":"admin@sotifibre.dz","password":"SOTIFibre@2026!"}'
```
Renvoie `{ "access": "...", "refresh": "..." }`.

### Utiliser le token
```bash
curl https://sotifibre-backend.onrender.com/api/users/users/ \
  -H "Authorization: Bearer <ACCESS_TOKEN>"
```

### Rafraîchir
```bash
curl -X POST https://sotifibre-backend.onrender.com/api/auth/jwt/refresh/ \
  -H "Content-Type: application/json" \
...
### Smoke test
```bash
curl https://sotifibre-backend.onrender.com/api/health/
# → {"status": "ok", "service": "sotifibre-backend"}
```
...
- **Root Directory** : `sotifibre_backend_django`
...
| `ALLOWED_HOSTS` | `sotifibre-backend.onrender.com` |
| `CORS_ALLOWED_ORIGINS` | `https://sotifibre-frontend.onrender.com` |
...
*Projet de Fin d'Études — Abdoulaye Adoum, 2026*
